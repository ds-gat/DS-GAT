...
def save_metrics_csv(trainer, model_name, dataset_name, output_dir="output"):
    ...
    filepath = os.path.join(output_dir, f"metrics_complex_{model_name}_{dataset_name}_{now}.csv")
...
def save_timing_csv(model_name, dataset_name, total_time_s, output_dir="output"):
    os.makedirs(output_dir, exist_ok=True)
    now      = datetime.now().strftime("%Y%m%d_%H%M%S")
    filepath = os.path.join(output_dir, f"timing_complex_{model_name}_{dataset_name}_{now}.csv")
...
def main(config_path, dataset_name):
    ...
    curves_callback.save(f"output/training_curves_complex_{file_tag}_{now}.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str,
                        default="config/ppi5k/PASSLEAF_ppi5kc.yaml")
