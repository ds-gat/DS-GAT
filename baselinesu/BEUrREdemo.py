# -*- coding: utf-8 -*-
"""BEUrRE demo — unKR library wrapper (box embeddings for uncertain KGs)."""
    ...
    def save(self, filepath):
        ...
        with open(filepath, 'w', newline='') as f:
            writer = csv.DictWriter(
                f, fieldnames=['epoch', 'train_loss', 'val_wmrr', 'val_wmr', 'val_mae'])
            writer.writeheader()
...
def save_metrics_csv(trainer, model_name, dataset_name, output_dir="results/baselines"):
    ...
    # BEUrRELitModel.test_epoch_end uses "Test" prefix
...
def main(config_path, dataset_name, seed=None):
    print(f"Running BEUrRE — config: {config_path}  dataset: {dataset_name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BEUrRE unKR runner")
    parser.add_argument("--config", type=str,
                        default="baselinesu/config/ppi5k/BEUrRE_ppi5k.yaml")
