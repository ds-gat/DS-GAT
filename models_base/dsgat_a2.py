"""DSGATA2 — Ablation: Deterministic edge weights (no Bayesian stream).

Identical to DSGAT5 except EGATConv28 is replaced with EGATConv_A2
which removes the stochastic Bayesian sampling and KL loss:
  - No correction_mlp
  - No variance_mlp / var_scale
  - No stochastic sampling at train time
  - No mean/var split — eff_w = clamp(s * softplus(mean_scale[r]), 1e-3, 1)
  - KL loss returns zero (no-op)

Stream B is KEPT — dual-stream aggregation unchanged.
Stream A is KEPT — softmax attention unchanged.

Purpose: validates contribution of Bayesian uncertainty modeling.
If DS-GAT >> DSGATA2, the Bayesian stream is a meaningful contribution.
Compare with DSGATA1 to isolate Stream B vs Bayesian stream contributions.
"""
...
class DSGATA2(nn.Module):
    def __init__(self, node_features, num_relations, embedding_dim, dropout,
        ...
        self.convs = nn.ModuleList([
            EGATConv_A2(node_dim, node_dim, node_dim, node_dim,
                        num_heads, num_relations)
            for _ in range(num_layers)
        ])

        # Still present for interface compatibility — always empty
    ...
    def forward(self, entity, edge_index, edge_type=None, edge_weight=None):
        ...
        for i, conv in enumerate(self.convs):
            x, efeats, w_det = conv(g, x, efeats, edge_type, edge_weight)
            x      = x.mean(dim=1)
            efeats = efeats.mean(dim=1)

            # Store deterministic w as (w, zeros) for interface compatibility
            self._conf_dist_cache.append((w_det, torch.zeros_like(w_det)))
    ...
    def confidence_kl_loss(self, edge_weight: torch.Tensor,
        ...
        """No-op — deterministic model has no KL loss."""
...
# ─────────────────────────────────────────────────────────────────────────────
# EGATConv_A2 — Deterministic edge weights, no Bayesian stream
# ─────────────────────────────────────────────────────────────────────────────

class EGATConv_A2(nn.Module):
    """EGATConv28 with Bayesian stream replaced by deterministic edge weights.

    Removed:
      - correction_mlp (delta correction)
      - edge_weight_var_mlp (variance prediction)
      - var_scale per-relation parameter
      - Stochastic sampling at train time
      - KL divergence loss contribution

    Kept:
      - mean_scale per-relation parameter (deterministic scaling)
      - Stream A: softmax attention (unchanged)
      - Stream B: lambda_raw + deterministic eff_w (unchanged structure)
      - edge_project: eff_w still injected into edge features

    Deterministic formula:
      eff_w = clamp(s * softplus(mean_scale[r]), 1e-3, 1.0)
      — same at train and eval time, no sampling gap
    """
    ...
    def __init__(self, in_node_feats, in_edge_feats, out_node_feats, out_edge_feats,
        ...
        # Bayesian stream REMOVED: no correction_mlp, no variance_mlp, no var_scale
        # Only mean_scale kept for deterministic per-relation scaling
    ...
    def _reset_parameters(self):
        ...
        for m in [self.fc_nodes, self.fc_edges, self.fc_attn,
                  self.edge_project, self.W_B]:
            init.xavier_normal_(m.weight, gain=gain)
        for m in [self.edge_project, self.W_B]:
            init.zeros_(m.bias)

    def _compute_eff_w(self, ew, edge_type):
        """Deterministic effective weight — raw confidence passed directly.
        eff_w = clamp(c_ij, 1e-3, 1.0)
        No correction, no scaling, no variance. Same at train and eval.
        Returns (eff_w, eff_w) for interface compatibility.
        """
        eff_w = torch.clamp(ew, 1e-3, 1.0)
        return eff_w, eff_w
    ...
    def forward(self, graph, nfeats, efeats, edge_type, edge_weight=None):
        """Returns (node_feats, edge_feats, eff_w) — no mu/var split."""
        with graph.local_scope():
            ...
            eff_w, _ = self._compute_eff_w(ew, edge_type)
            ...
            graph.edata.update({'f': ef, 'f_b': ef, 'lam_w': lam_w})
            ...
            return graph.ndata.pop('h'), graph.edata.pop('f'), eff_w