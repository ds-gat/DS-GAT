...
class RGAT(torch.nn.Module):
    def __init__(self,
        ...
        # Entity embedding (trainable)
        self.entity_embedding = nn.Embedding.from_pretrained(
            node_features,   # shape: [num_entities, feature_dim]
            freeze=False
        )

        # Relation embeddings for DistMult
        ...
        # Project pretrained features → latent space
        ...
        for _ in range(num_layers):
            self.convs.append(
                RGATConv(
                    ...
                    num_bases=num_bases,
                    heads=heads,
                    concat=True,  # keep same dimensionality
                    dropout=dropout,
                    edge_dim=1
                )
            )
    ...
    def forward(self, entity, edge_index, edge_type, edge_weight=None):
        ...
        edge_type: [num_edges]
        edge_weight: optional -> passed as edge_attr
        """
